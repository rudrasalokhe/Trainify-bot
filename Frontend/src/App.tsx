...
function App() {
  ...
  return (
    <div className={`flex h-screen ${darkMode ? 'bg-gradient-to-br from-zinc-950 to-zinc-900 text-zinc-100' : 'bg-gradient-to-br from-gray-50 to-gray-100 text-gray-900'}`}>
      <SignedOut>
      ...
      <SignedIn>
        {/* Mobile header */}
        <div className={`md:hidden fixed top-0 left-0 right-0 z-30 p-3 flex justify-between items-center ${darkMode ? 'bg-zinc-900/90 border-b border-zinc-800 backdrop-blur-sm' : 'bg-white/90 border-b border-gray-200 backdrop-blur-sm'}`}>
          <button 
        ...
        <motion.div 
          ...
          className={`fixed md:relative z-20 h-full ${showSidebar ? 'w-72' : 'w-0'} ${darkMode ? 'bg-zinc-900/95 border-zinc-800' : 'bg-white/95 border-gray-200'} border-r flex flex-col overflow-hidden backdrop-blur-sm`}
        >
          <div className={`p-4 border-b flex justify-between items-center ${darkMode ? 'border-zinc-800' : 'border-gray-200'} mt-12 md:mt-0`}>
            <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2">
              <button 
                ...
                className={`p-2 rounded-full transition-all ${darkMode ? 'hover:bg-zinc-800' : 'hover:bg-gray-100'}`}
              >
                {darkMode ? (
          ...
          <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-zinc-700/50 scrollbar-track-transparent pb-20 md:pb-0">
            {chats.length === 0 ? (
              ...
              Object.entries(groupedChats).map(([date, dateChats]) => (
                <div key={date}>
                  <div className={`px-4 py-2 text-xs font-medium ${darkMode ? 'text-zinc-500' : 'text-gray-500'}`}>
                    {date}
                  </div>
                  {dateChats.map(chat => (
                    <motion.div 
                      key={chat.id}
                      whileHover={{ x: 5 }}
                      className={`group flex items-center justify-between p-3 cursor-pointer transition-all duration-150 ${currentChat?.id === chat.id ? (darkMode ? 'bg-zinc-800/80 border-l-4 border-blue-500' : 'bg-blue-50/80 border-l-4 border-blue-500') : (darkMode ? 'hover:bg-zinc-800/50' : 'hover:bg-gray-100/80')}`}
                      onClick={() => {
                        setCurrentChat(chat);
                        setShowMobileMenu(false);
                      }}
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1 mt-1">
                          ...
                          <span className={`text-xs ${chat.modelType === 'advanced' ? 'text-purple-400' : (darkMode ? 'text-zinc-500' : 'text-gray-500')}`}>
                      ...
                      <div className="flex items-center gap-1">
                        <button
                          ...
                          className={`${favoriteChats.includes(chat.id) ? '' : 'opacity-0 group-hover:opacity-100'} p-1 rounded transition-all ${darkMode ? 'hover:bg-zinc-700' : 'hover:bg-gray-200'} ${favoriteChats.includes(chat.id) ? 'text-yellow-500' : 'hover:text-yellow-500'}`}
                        ...
                        <button
                          ...
                          className={`opacity-0 group-hover:opacity-100 p-1 rounded transition-all ${darkMode ? 'hover:bg-zinc-700 hover:text-blue-400' : 'hover:bg-gray-200 hover:text-blue-600'}`}
                        ...
                        <button
                          ...
                          className={`opacity-0 group-hover:opacity-100 p-1 rounded transition-all ${darkMode ? 'hover:bg-zinc-700 hover:text-red-400' : 'hover:bg-gray-200 hover:text-red-600'}`}
                          title="Delete chat"
                        >
                          <Trash2 size={16} />
                        </button>
        ...
        <div className="flex-1 flex flex-col pt-12 md:pt-0">
          {currentChat ? (
            <>
              <div className={`p-4 border-b flex justify-between items-center ${darkMode ? 'border-zinc-800 bg-zinc-900/80' : 'border-gray-200 bg-white/80'} backdrop-blur-sm`}>
                <div className="flex items-center gap-2">
                  <button 
                    onClick={() => setShowMobileMenu(!showMobileMenu)} 
                ...
                <div className="flex items-center gap-3">
                  <div className="relative">
                    <button
                      ...
                      className={`flex items-center gap-1 rounded-lg px-3 py-1.5 border text-sm transition-all ${darkMode ? 'bg-zinc-800 text-zinc-100 border-zinc-700 hover:bg-zinc-700' : 'bg-white text-gray-800 border-gray-200 hover:bg-gray-100'}`}
                    >
                      <Languages size={16} />
                    ...
                    {showLanguageDropdown && (
                      <motion.div
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: -10 }}
                        className={`absolute right-0 mt-1 w-48 rounded-lg shadow-lg z-10 overflow-hidden ${darkMode ? 'bg-zinc-800 border-zinc-700' : 'bg-white border-gray-200'} border`}
                      >
                        {languages.map((lang) => (
                          <button
                            key={lang.value}
                            onClick={() => handleLanguageChange(lang.value)}
                            className={`w-full text-left px-4 py-2 text-sm flex items-center justify-between hover:bg-blue-500 hover:text-white transition-colors ${darkMode ? 'hover:bg-blue-600' : 'hover:bg-blue-500'} ${currentChat.language === lang.value ? (darkMode ? 'bg-blue-600 text-white' : 'bg-blue-500 text-white') : (darkMode ? 'text-zinc-100' : 'text-gray-800')}`}
                          >
                  ...
                  <select
                    ...
                    className={`rounded-lg px-3 py-1.5 border text-sm focus:outline-none transition-all ${darkMode ? 'bg-zinc-800 text-zinc-100 border-zinc-700 hover:bg-zinc-700' : 'bg-white text-gray-800 border-gray-200 hover:bg-gray-100'}`}
              ...
              <div className="flex-1 overflow-y-auto p-4 pb-24 md:pb-4">
                {currentChat.messages.length === 0 ? (
                  ...
                  >
                    <div className={`p-6 rounded-full mb-6 ${darkMode ? 'bg-zinc-800' : 'bg-gray-100'} shadow-inner`}>
                      <MessageSquarePlus size={32} className={darkMode ? 'text-zinc-500' : 'text-gray-400'} />
                    </div>
                    <h3 className="text-xl font-medium mb-2">Start a conversation</h3>
                    <p className={`max-w-md mb-6 ${darkMode ? 'text-zinc-400' : 'text-gray-500'}`}>
                  ...
                  <div className="space-y-6">
                    {currentChat.messages.map((message) => (
                      ...
                      >
                        <div className={`flex items-start gap-3 ${message.sender === 'user' ? 'flex-row-reverse' : ''}`}>
                          <div className={`mt-1 p-2 rounded-full ${message.sender === 'user' ? 'bg-blue-600 text-white' : darkMode ? 'bg-zinc-800 text-blue-400' : 'bg-gray-200 text-blue-600'}`}>
                            {message.sender === 'user' ? <UserIcon size={16} /> : <Bot size={16} />}
                          </div>
                          <div className={`max-w-[90%] md:max-w-3xl rounded-lg p-4 relative transition-all ${message.sender === 'user' ? darkMode ? 'bg-blue-600 text-white' : 'bg-blue-600 text-white' : darkMode ? 'bg-zinc-800 text-zinc-100' : 'bg-white text-gray-800 border border-gray-200'}`}>
                            {message.fileInfo && (
                              <div className={`text-xs mb-2 p-2 rounded ${darkMode ? 'bg-zinc-700' : 'bg-gray-100'}`}>
                                <div className="flex items-center gap-1">
                                  <FileText size={14} />
                            ...
                            <div className={`absolute text-xs ${message.sender === 'user' ? darkMode ? 'text-blue-300' : 'text-blue-100' : darkMode ? 'text-zinc-500' : 'text-gray-500'} bottom-1 right-2`}>
                            ...
                            <div className={`absolute flex gap-1 ${message.sender === 'user' ? '-left-10' : '-right-10'} top-0 opacity-0 group-hover:opacity-100 transition-opacity`}>
                              <button
                                ...
                                className={`p-1.5 rounded-full ${darkMode ? 'hover:bg-zinc-700 hover:text-blue-400' : 'hover:bg-gray-200 hover:text-blue-600'}`}
                                title="Copy message"
                              >
                                <Copy size={16} />
                              </button>
                              {message.sender === 'user' && (
                                <button
                                  ...
                                  className={`p-1.5 rounded-full ${darkMode ? 'hover:bg-zinc-700 hover:text-red-400' : 'hover:bg-gray-200 hover:text-red-600'}`}
                            ...
                            {copiedMessageId === message.id && (
                              <motion.div
                                ...
                                animate={{ opacity: 1, y: 0 }}
                                exit={{ opacity: 0, y: -5 }}
                                className={`absolute -top-8 left-1/2 transform -translate-x-1/2 px-2 py-1 rounded text-xs ${darkMode ? 'bg-zinc-700' : 'bg-gray-200'}`}
                              >
                                Copied!
                              </motion.div>
                            )}
                    ...
                    {isLoading && (
                      ...
                      >
                        <div className="mt-1 p-2 rounded-full bg-zinc-800 text-blue-400">
                          <Bot size={16} />
                        </div>
                        <div className={`max-w-[90%] md:max-w-3xl rounded-lg p-4 ${darkMode ? 'bg-zinc-800' : 'bg-white border border-gray-200'}`}>
                          <LoadingDots />
                        </div>
                      </motion.div>
                    )}
              ...
              <div className={`fixed bottom-0 left-0 right-0 md:relative p-4 border-t ${darkMode ? 'border-zinc-800 bg-zinc-900/80' : 'border-gray-200 bg-white/80'} backdrop-blur-sm`}>
                <form onSubmit={sendMessage} className="flex gap-2">
                  <div className="flex-1 relative">
                    <input
                      ...
                      className={`w-full rounded-lg py-3 px-4 pr-12 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all ${darkMode ? 'bg-zinc-800 text-white placeholder-zinc-400' : 'bg-white text-gray-900 placeholder-gray-400 border border-gray-200'}`}
                    ...
                    <div className="absolute right-2 top-2 flex gap-1">
                      <button
                        ...
                        disabled={isLoading || isUploading}
                        className={`p-2 rounded-full transition-all ${darkMode ? 'hover:bg-zinc-700 text-zinc-400 hover:text-zinc-300' : 'hover:bg-gray-100 text-gray-500 hover:text-gray-600'}`}
                        title="Upload file"
                      >
                        <Upload size={20} />
                      </button>
                  ...
                  <motion.button
                    ...
                    className={`p-3 rounded-lg flex items-center justify-center transition-all ${inputMessage.trim() && !isLoading && !isUploading ? 'bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-700 hover:to-blue-600 text-white shadow-md' : darkMode ? 'bg-zinc-800 text-zinc-500' : 'bg-gray-200 text-gray-500'}`}
                  >
                ...
                <div className={`text-xs mt-2 text-center ${darkMode ? 'text-zinc-500' : 'text-gray-500'}`}>
            ...
            >
              ...
              <p className={`max-w-md text-center mb-6 px-4 ${darkMode ? 'text-zinc-400' : 'text-gray-500'}`}>
